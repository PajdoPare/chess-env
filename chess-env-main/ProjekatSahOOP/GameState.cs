...
namespace ProjekatSahOOP
{
    ...
    public class GameState
    {
        ...
        Potez Sklopi(Kvadrat P, Kvadrat O, Piece Promotion = null)
        {
            ...
            m.Beli = p.beli;
            if (Board.GetPiece(O) != null) m.Capture = true;
            if (p.T == Tip.Kralj && DaLiJeRokada(P, O))
            {
                m.Rokada = true;
                bool desno = (O.Col == 6);
        ...
        private string SahSuf()
        {
            if (St == Status.Sah) return "+";
            if (St == Status.Mat) return "#";
            return "";
        }
        private static char PieceChar(Piece p)
        {
            ...
            switch (p.T)
            {
                case Tip.Kralj: c = 'K'; break;
                case Tip.Kraljica: c = 'Q'; break;
                case Tip.Top: c = 'R'; break;
                case Tip.Lovac: c = 'B'; break;
                case Tip.Skakac: c = 'N'; break;
                case Tip.Pesak: c = 'P'; break;
                default: c = ' '; break;
            }
            return c;
        }
        public string Notacija(Potez m)
        {
            if (m.Rokada)
            {
                string rokada = m.Odredisno.Col == 6 ? "O-O" : "O-O-O";
                return rokada + SahSuf();
            }
            string S = "";
            if (Board.GetPiece(m.Odredisno).T != Tip.Pesak) S += PieceChar(Board.GetPiece(m.Odredisno));
            S += Duplikati(m);
            if(m.Capture)
            {
                if (Board.GetPiece(m.Odredisno).T == Tip.Pesak) S += (char)('a' + m.Polazno.Col);
                S += 'x';
            }
            S += m.Odredisno.ToString();
            if (m.Promo) S += $"= {PieceChar(m.Promocija)}";
            
            S += SahSuf();
            return S;
        }
        private string Duplikati(Potez m)
        {
            if(Board.GetPiece(m.Odredisno).T == Tip.Pesak || Board.GetPiece(m.Odredisno).T == Tip.Kralj)return "";
            List<Kvadrat> list = new List<Kvadrat>();
            for (int i = 0; i < 8; i++)
            {
                for(int j = 0; j < 8; j++)
                {
                    if(i == m.Polazno.Row && j == m.Polazno.Col) continue;
                    Piece p = Board.GetPiece(i, j);
                    if(p == null || p.T != Board.GetPiece(m.Odredisno).T || p.beli != Board.GetPiece(m.Odredisno).beli) continue;
                    Kvadrat k = new Kvadrat(i, j);
                    if(LegalMoves(k).Contains(m.Odredisno))list.Add(k);
                }
            }
            ...
            {
                if (list[i].Row == m.Polazno.Row) red = true;
                if (list[i].Col == m.Polazno.Col) col = true;
            }
            if(!red)return ((char)('a' + m.Polazno.Col)).ToString();
            if (!col) return (8 - m.Polazno.Row).ToString();
            return m.Polazno.ToString();
        }
    }
}